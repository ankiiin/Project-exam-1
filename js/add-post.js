document.getElementById("post-form").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            errorMessage.textContent = errorData.errors ? errorData.errors[0].message : "Failed to add post.";
            errorMessage.style.display = "block";
        }
    } catch (error) {
        console.error("Error adding post:", error);
        errorMessage.textContent = "An error occurred while adding the post. Please try again.";
        errorMessage.style.display = "block";
    }
});
